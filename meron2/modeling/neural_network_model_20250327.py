...
NUM_EPOCHS = 150  # More epochs but with early stopping
LEARNING_RATE = 0.001  # Higher learning rate for Adam
EARLY_STOPPING_PATIENCE = 20  # Longer patience
VALIDATION_SPLIT = 0.2  # For creating validation set
...
def create_balanced_sampler(labels, alpha=0.5):
    """
...
def main():
    ...
    # Create a partially balanced sampler (not too aggressive)
    print("\nCreating partially balanced sampler (alpha=0.75)...")
    train_sampler = create_balanced_sampler(y_train, alpha=0.75)
    ...
    class_weights = torch.FloatTensor([1.0, 2.0, 3.0]).to(device)
    print(f"\nModerate class weights: {class_weights}")
    
    # Create loss function
    print("\nSetting up loss function and optimizer...")
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    
    # Use Adam with moderate weight decay
    ...
    # Learning rate scheduler - reduce on plateau is safer
    ...
    print("\nOptimization approach: Balanced architecture with moderate regularization")
    print("Using moderate class weights and Adam optimizer")
    print("Using ReduceLROnPlateau scheduler to adjust learning rate")
