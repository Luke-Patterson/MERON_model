...
def visualize_results(cm, accuracy, f1, training_history, results_dir, experiment_id, probs, labels):
    ...
    plt.tight_layout()
    plt.savefig(results_dir / f'resnet50_confusion_matrix_{experiment_id}.png')
    ...
    plt.tight_layout()
    plt.savefig(results_dir / f'resnet50_training_history_{experiment_id}.png')
    ...
    plt.tight_layout()
    plt.savefig(results_dir / f'resnet50_roc_curve_{experiment_id}.png')
    
    # Generate and save classification report as CSV
    report = classification_report(labels, (np.array(probs) > CONFIG['PREDICTION_THRESHOLD']).astype(int), output_dict=True)
    report_df = pd.DataFrame(report).transpose()
    report_df.to_csv(results_dir / f'resnet50_classification_report_{experiment_id}.csv')
    ...
    with open(results_dir / f'resnet50_metrics_{experiment_id}.json', 'w') as f:
...
def main():
    ...
    # Load best model
    checkpoint = torch.load(model_dir / f'best_model_{experiment_id}.pth', weights_only=False)
